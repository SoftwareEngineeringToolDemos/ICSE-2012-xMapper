1. Once the OS starts up, eclipse is also started in the ArchStudio perspective.

2. Find the architecture file arch.xml in the navigator view and right click it. Select Open With -> Archipelago.

3. Right click the element Structures in the Outline view and double click the Chatsys element to open it in Archipelago editor.

4. Now we can proceed as per the "VM xMapper An Architecture-Implementation Mapping Tool Demo - YouTube" link shared on the VM desktop to test the xMapper tool.

5. To open the architecture file arch.xml in ArchEdit, right click the arch.xml in the navigator view and select Open With -> ArchEdit.
